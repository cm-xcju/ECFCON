...
def get_optim_for_emocas(opt, model):
    optim_class = Optim.AdamW
    params = [
        {'params': [], 'lr': opt.LR_BASE, '_grad_norm_clip': opt.GRAD_NORM_CLIP},
        {'params': [], 'lr': opt.LR_BASE* opt.BERT_LR_MULT, '_grad_norm_clip': opt.GRAD_NORM_CLIP},
    ]
    ...
    hyper_params = {k: eval(v) for k, v in opt.OPT_PARAMS.items()}
    return OptimizerWrapper(
        [optim_class(
            params,
            **hyper_params
        ),],
        warmup_schd_steps=opt.WARMUP_EPOCH,
        decay_schd_step_list=opt.LR_DECAY_LIST,
        decay_rate=opt.LR_DECAY_R,
    )
