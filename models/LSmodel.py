...
class LSmodel(nn.Module):
    def __init__(self, opt,emo2idx,cos2idx) -> None:
        ...
        self.hsize=500
        self.embedding = nn.Embedding(len(self.tokenizer),opt.hidden_size)
        self.word_rnn = nn.LSTM(input_size=opt.hidden_size,hidden_size=opt.hidden_size,num_layers=1,batch_first=True)
        self.flaten = nn.Linear(opt.hidden_size,opt.hidden_size)
    ...
    def forward(self,Speaker_input_ids, Speaker_input_attention_mask, input_ids, input_attention_mask,\
        ...
        pooler_output=self.get_lstm_pooler_output(spk_and_input_ids)
        ...
        if opt.select_modality =='TAVC':
            # for clues 
            if self.opt.task != 'CF':
                emo_clue_pooler_output= self.get_lstm_pooler_output(emo_clue_input_ids)
            cos_clue_why_pooler_output= self.get_lstm_pooler_output(cos_clue_why_input_ids)
            cos_clue_impact_pooler_output= self.get_lstm_pooler_output(cos_clue_impact_input_ids)
            # if self.opt.task != 'CF':
    ...
    def get_lstm_pooler_output(self,input_ids):
        batch_size,seq_len,_ =input_ids.shape
        dense_spk_and_input_ids = input_ids.reshape(-1,input_ids.shape[-1])
        si_emb = self.embedding(dense_spk_and_input_ids)
        si_word_feas,*_ = self.word_rnn(si_emb)
        si_word_fea = si_word_feas[:,-1,:]
        si_word_fea = si_word_fea.reshape(batch_size,seq_len,-1)
        pooler_output = self.flaten(si_word_fea)
        return pooler_output
...
#         self.tokenizer = AutoTokenizer.from_pretrained(opt.bert_path)
#         self.embedding = nn.Embedding(len(self.tokenizer),opt.hidden_size)
#         self.word_rnn = nn.LSTM(input_size=opt.hidden_size,hidden_size=opt.hidden_size,num_layers=1,batch_first=True,bidirectional=True)
#         self.flaten = nn.Linear(opt.hidden_size*2,opt.hidden_size)
#         if opt.select_modality =='T':
#             self.simple_fusion = nn.Sequential(
#                 nn.LayerNorm(opt.hidden_size),
#                 nn.Linear(opt.hidden_size, 2*opt.hidden_size),
